...
describe(runUpdateBareConfigsCommand.name, () => {
    ...
    it('should not write config files when there are errors', async () => {
        /** Since this folder is literally empty, it doesn't get added to git and we have to recreate it. */
        ...
        if (alreadyExistingByKey.length) {
            console.error(`Already existing bare configs: ${alreadyExistingByKey.join(', ')}`);
        }
        ...
        if (!commandOutput.success) {
            console.error(
                `Update bare configs command failed: ${JSON.stringify(commandOutput, null, 4)}`,
            );
        }
        ...
        const {writtenConfigs, invalidConfigs} = allBareConfigKeys.reduce(
            (accum, configKey) => {
                ...
                const writtenSuccessfully = stdoutHasKey && !stderrHasKey && configWasWritten;
        ...
        const failures = [
            ...
            ...writtenConfigs.filter((writtenConfig, index) => {
                if (alreadyExistingByKey.includes(writtenConfig)) {
                    console.error(`"${writtenConfig}" already existed before running the test`);
                    return true;
                }
